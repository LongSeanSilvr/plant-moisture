...
# --- 1. UNMISTAKABLE STARTUP BLINK (5 fast blinks) ---
pixel = digitalio.DigitalInOut(board.NEOPIXEL)
pixel.direction = digitalio.Direction.OUTPUT
print("Blinking Diagnostic...")
for _ in range(5):
    pixel.value = True; time.sleep(0.1)
    pixel.value = False; time.sleep(0.1)

# --- 2. STABILIZATION DELAY (10s) ---
...
SPRITE_DATA = [
    ...
    "",
...
class PlantMonitor:
    ...
    def run(self):
        ...
        while True:
            ...
            if (now - last_rotate >= ROTATION_INTERVAL) and (len(self.plants) > self._max_visible):
