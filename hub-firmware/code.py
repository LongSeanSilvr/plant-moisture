...
# --- STABILIZATION ---
# 1. Visual Heartbeat & Boot Delay (Ensures stable power/USB before heavy init)
...
# FULL PREMIUM SPRITE DATA (Restored from previous best version)
SPRITE_DATA = [
    # VARIANT 1: Fig
...
class PlantUI:
    ...
    def update(self, value):
        ...
        if value < 20: state = critical_idx
        elif value < 50: state = thirsty_idx
        else: state = self.assigned_variant
        ...
        try:
            ...
            for f in feeds:
                ...
                if key.endswith('-moisture'):
                    ...
                    self.plants.append(PlantUI(label_name, key, self.font, self.sheet, self.palette, variant=cfg.get("variant")))
